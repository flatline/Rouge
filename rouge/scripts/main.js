...
function Map() {
     ...
     * @param idx - optional; if present, injects at the specified z-index of the stack, else pushes to the top
     * @return
     */
    ...
    /**
     * The most basic movement operation.  It enforces an interface on the target cell
     * such that tryToPass, if defined, is called on all objects in the target object 
     * stack.  If tryToPass does not fail for any items in the target, the object's 
     * cell reference is updated, completing the move.
     */
    this.move = function(obj, row, col, idx) {    	
       	if (row >= this.height || row < 0 || col >= this.width || col < 0) return self;    	
       	var cell = self.peek(row, col);    	
    ...
    this.distance = function (loc1, loc2) {
  		    try {
 			        return Math.round(Math.sqrt( Math.pow(loc1.row - loc2.row, 2) + Math.pow(loc1.col - loc2.col, 2)));
  		    } catch (e) {
    ...
    /**
     * Utility to print out the full map representation for debugging
     */  
    this.toString = function() {
        return self._grid.fold(function(row, accr) {
            return accr + "\r\n" + row.fold(function(cell, accc) { 
                if (cell.length == 0) return accc + "[ ]";
                else return accc + cell.fold(function(item, acci) {
                    return acci + item.repr;
                }, "[") + "]";                        
            }, "");
        }, "");
    };
