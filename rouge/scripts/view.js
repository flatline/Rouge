...
var gameFrameBuilder = new function GameFrameBuilder() {
    this.build = function(map, imageTable, backgroundImage, attrs) {
      		    ...
      		    window.setTimeout(function() {
      		        ...
      		        for (var i = 0; i < map.height; i++) {
       	            ...
       	            for (var j = 0; j < map.width; j++) {
       	                var cell = row[j];
       	                var ground = cell[0];
       	                var statics = cell.filter(function (item) { return item.hasOwnProperty("static") && item["static"] === true });
        ...
        frame.draw = function(ctx) {
            ctx.drawImage(backgroundImage, frame.playerXOffset(), frame.playerYOffset());
        ...
        frame.playerXOffset = function() {
           	return frame.left - (player.loc.col * vc.tileWidth) + (frame.width / 2) - vc.tileWidth/2;
        };
    
        frame.playerYOffset = function() {
           	return frame.top - (player.loc.row * vc.tileHeight) + (frame.height / 2) - vc.tileHeight/2;
        };
        ...
        frame.showFPS = function(ctx) {
            ...
            var avg = timings.fold(function(i, acc) { return i + acc; }, 0) / timings.length;
...
var messageFrameBuilder = new function MessageFrameBuilder() {
    this.build = function(map, frameAttrs) {
        ...
        msgFrame.draw = function(ctx) {
            ...
            var len = messages.length > maxMessages ? maxMessages : messages.length;
            ...
            if (messages.length > maxMessages) map.messages = messages.slice(messages.length - maxMessages, messages.length + 1);
        };
...
function HtmlMapView(controller) {	
    ...
    var mainFrame, gameFrame, messageFrame = null;
   	...
   	this.init = function() {
   	    //create the main canvas and a hidden canvas for pre-rendering the background.
      		var canvas = $("<canvas width='" + vc.displayWidth + "px' height='" + vc.displayHeight + "px'>Incompatible browser</canvas>");		
  		    ...
  		    $("body").append('<div id="debug" style="width: 100%; overflow-y: scroll; height: 300px"><ul></ul></div>');
        ...
        self.initFrames();
  		    ...
  		    //preload images - because we don't have any way to properly wait, we have to 
       	...
       	display.fillText("Loading images...", 200, 144);
  		    ...
  		    (new EventChain())
 			        ...
 			        .add(self.gameFrame.prerenderBackground, self.gameFrame, "prerenderComplete")
    ...
    //	    			if (txt != "") {
    //	    				this.userInput.text("");
    //	    				//TODO: handle input, handle keypress when in textbox earlier too
    //	    			}
    //	    			break;
            case 83: // s - save game
                controller.saveGame();
                ...
                break;
     	    		default:
     	    		    ...
     	    		    controller.setPlayerCommand(playerCommands["k" + evt.keyCode.toString()]);	    			
        	    			break;    			
      	    	}
        });				
	   };
