...
function Controller(map) {
    ...
    /**
     * Sets the player's action based on an ActionBuilder (from player.js/playerCommands)
     * Was formerly in the view object
     */
    this.setPlayerCommand = function(actionBuilder) {	   
	    			if (typeof(actionBuilder) !== "undefined") {
				        //TODO: aren't some of these parameters now redundant after moving the body
    				    //of this code into player.setAction?
    				    self.map.player.setAction(actionBuilder(self.map, self.map.player), self);
	    			}
	   }
	   ...
	   this.saveGame = function() {
        ...
        if (!self.hasOwnProperty("gameId")) self.gameId = parseInt(Math.random() * 10000);
