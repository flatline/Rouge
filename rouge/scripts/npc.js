function NPC(spec) {
    ...
    this.typeName = "NPC";
   	...
   	this.getNextAction = function(ctrl) {
  		    ...
  		    //TODO: build a better model for state transitions and strategy selection.
  		    if (dist <= 7 && dist > 1) {
 			        if (sName !== "FollowStrategy") self.setCurrentStrategy(new FollowStrategy(self, map.player), "FollowStrategy");			
  		    } else if (dist <= 1) {
 			        if (sName !== "CombatStrategy") self.setCurrentStrategy(new CombatStrategy(self, map.player), "CombatStrategy");
  		    } else {
 			        if (sName !== "WanderStrategy") self.setCurrentStrategy(new WanderStrategy(self), "WanderStrategy");
  		    }
...
function WanderStrategy(actor) {
   	...
   	this.move = function(map, vec) {
		      map.move(self.actor, self.actor.loc.row + vec[0], self.actor.loc.col + vec[1]);
	   };
}
